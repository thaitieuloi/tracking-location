...
import { Clock, Navigation2, CalendarDays } from "lucide-react";
import { format } from "date-fns";

const dotIcon = L.divIcon({
  className: 'bg-transparent',
  html: `<div class="w-3 h-3 rounded-full bg-primary border-2 border-white shadow-md"></div>`,
  iconSize: [12, 12]
});
...
export default function HistoryPage() {
  ...
  const familyId = family?.family.id || "demo";
  ...
  const { data: historyPoints, isLoading } = useGetMemberLocationHistory(activeUserId, { hours });
    ...
    <AppLayout>
      <div className="flex flex-col md:flex-row h-full">
        ...
        <div className="w-full md:w-80 bg-card border-r border-border/50 flex flex-col z-10 shadow-lg">
          <div className="p-6 border-b border-border/50">
            <h2 className="text-2xl font-display font-bold flex items-center gap-2">
              <HistoryIcon className="text-primary w-6 h-6" /> Location History
            </h2>
            <p className="text-muted-foreground text-sm mt-1">Track past movements</p>
          </div>
          ...
          <div className="p-4 space-y-6 flex-1 overflow-y-auto">
            ...
            {isLoading ? (
              ...
              <div className="py-12 text-center text-muted-foreground flex flex-col items-center">
                ...
                <p>No location data found<br/>for this time range.</p>
              </div>
            )}
          </div>
        </div>
        ...
        <div className="flex-1 relative z-0 min-h-[50vh]">
          <MapContainer center={[10.762622, 106.660172]} zoom={13} className="w-full h-full" zoomControl={false}>
            <TileLayer url="https://{s}.basemaps.cartocdn.com/rastertiles/voyager/{z}/{x}/{y}{r}.png" />
            
            {historyPoints && historyPoints.length > 0 && (
              <>
                <Polyline 
                  positions={historyPoints.map(p => [p.latitude, p.longitude])} 
                  pathOptions={{ color: 'hsl(var(--primary))', weight: 4, opacity: 0.8, lineCap: 'round', lineJoin: 'round' }} 
                />
                
                {historyPoints.filter((_, i) => i % 5 === 0).map(p => (
                  <Marker key={p.id} position={[p.latitude, p.longitude]} icon={dotIcon}>
                    <Popup>
                      <div className="text-sm">
                        <p className="font-bold">{format(new Date(p.recorded_at), 'MMM d, h:mm a')}</p>
                        <p className="text-muted-foreground">{Math.round(p.speed || 0)} km/h</p>
                      </div>
                    </Popup>
                  </Marker>
                ))}
